...
async function login(email, password) {
    try {
        ...
        // Generate a token
        const refressToken = jwt.sign({ user_id: user.id, is_admin: user.is_admin }, process.env.REFRESH_TOKEN_KEY, {
            expiresIn: '7d',
        });
...
async function refresh(userId) {
    ...
    // Generate a token
    const accessToken = jwt.sign({ user_id: user.id, isAdmin: user.is_admin }, process.env.ACCESS_TOKEN_KEY, {
        expiresIn: '1d',
    });
